...
export default {
  ...
  methods: {
	...
	this.ruleForm.shenqingbeizhu = this.ruleForm.shenqingbeizhu.replace(reg,'../../../springboot7l0u0/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `songcangshenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.songcangshenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
