    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="预订备注" prop="yudingbeizhu">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.yudingbeizhu" label="预订备注" prop="yudingbeizhu">
                    <span v-html="ruleForm.yudingbeizhu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.yudingbeizhu = this.ruleForm.yudingbeizhu.replace(reg,'../../../springboot7l0u0/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shangpinyuding/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shangpinyudingCrossAddOrUpdateFlag = false;
						 this.parent.search();
